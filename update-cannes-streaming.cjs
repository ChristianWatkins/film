...
async function getCannesFilms() {
  ...
  try {
    ...
    console.error('Error reading Cannes festival files:', error.message);
    throw error;
  }
}
...
async function main() {
  console.log('🎬 Updating Streaming Data for Cannes Films');
  console.log('='.repeat(60));
  
  // Initialize JustWatch client
  const justwatch = new JustWatchAPI(10000); // 10 second timeout
  
  // Load Cannes films and master films
  console.log('📂 Loading Cannes festival films...');
  const cannesFilms = await getCannesFilms();
  console.log(`✓ Found ${cannesFilms.length} Cannes film entries\n`);
  ...
  for (const cannesFilm of cannesFilms) {
    const masterFilm = masterFilms[cannesFilm.id];
    
    if (!masterFilm) {
      console.log(`[${processed + 1}/${cannesFilms.length}] ⚠️  Film ID ${cannesFilm.id} not found in master films, skipping`);
      skipped++;
      continue;
    }
    
    processed++;
    console.log(`[${processed}/${cannesFilms.length}] ${masterFilm.title} (${masterFilm.year})`);
    
    const result = await fetchFilmStreaming(justwatch, cannesFilm, masterFilm);
    
    // Store by film ID (short code)
    streamingData.films[cannesFilm.id] = result;
    ...
    if (processed % 20 === 0) {
      ...
      console.log(`  💾 Progress saved (${processed}/${cannesFilms.length})\n`);
    }
    
    // Delay before next request (except for last one)
    if (processed < cannesFilms.length) {
  ...
  console.log('📊 Summary:');
  console.log('='.repeat(60));
  console.log(`Total Cannes films: ${cannesFilms.length}`);
