...
const FESTIVAL_NAME = 'berlin'; // Used for folder name
const YEAR = 2021; // Change this to scrape different years
    ...
    const films = await page.evaluate(() => {
      ...
      return Array.from(filmElements).map(element => {
        ...
        const awarded = !!(element.querySelector('.award, .winner, .prize') || 
                          ...
                          element.textContent.toLowerCase().includes('golden bear') ||
                          element.textContent.toLowerCase().includes('silver bear'));
        
        // Extract specific awards
        const awards = [];
        const awardText = element.textContent.toLowerCase();
        if (awardText.includes('golden bear')) awards.push('Golden Bear');
        if (awardText.includes('silver bear')) awards.push('Silver Bear');
        if (awardText.includes('jury prize')) awards.push('Jury Prize');
        if (awardText.includes('grand prix')) awards.push('Grand Prix');
...
async function scrapeYear(year) {
  console.log(`Starting to scrape Berlin International Film Festival ${year}...`);
  ...
  try {
    ...
    console.log(`\nScraping complete! Found ${allFilms.length} total films for Berlin ${year}`);
