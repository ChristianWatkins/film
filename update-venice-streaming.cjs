...
async function getVeniceFilms() {
  ...
  try {
    ...
    console.error('Error reading Venice festival files:', error.message);
    throw error;
  }
}
...
async function main() {
  console.log('🎬 Updating Streaming Data for Venice Films');
  console.log('='.repeat(60));
  
  // Initialize JustWatch client
  const justwatch = new JustWatchAPI(10000); // 10 second timeout
  
  // Load Venice films and master films
  console.log('📂 Loading Venice festival films...');
  const veniceFilms = await getVeniceFilms();
  console.log(`✓ Found ${veniceFilms.length} Venice film entries\n`);
  ...
  for (const veniceFilm of veniceFilms) {
    const masterFilm = masterFilms[veniceFilm.id];
    
    if (!masterFilm) {
      console.log(`[${processed + 1}/${veniceFilms.length}] ⚠️  Film ID ${veniceFilm.id} not found in master films, skipping`);
      skipped++;
      continue;
    }
    
    processed++;
    console.log(`[${processed}/${veniceFilms.length}] ${masterFilm.title} (${masterFilm.year})`);
    
    const result = await fetchFilmStreaming(justwatch, veniceFilm, masterFilm);
    
    // Store by film ID (short code)
    streamingData.films[veniceFilm.id] = result;
    ...
    if (processed % 20 === 0) {
      ...
      console.log(`  💾 Progress saved (${processed}/${veniceFilms.length})\n`);
    }
    
    // Delay before next request (except for last one)
    if (processed < veniceFilms.length) {
  ...
  console.log('📊 Summary:');
  console.log('='.repeat(60));
  console.log(`Total Venice films: ${veniceFilms.length}`);
